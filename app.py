...
# Manual CORS headers on every response (proven to work on Render)
@app.after_request
...
@app.route('/')
def home():
    return "AZEX PestGuard Backend is LIVE! Full features enabled"
...
def get_employees():
    ...
    return jsonify([{
        ...
        'photo': f"/uploads/employees/{e.photo}" if e.photo else None,
...
# Add other routes (customers CRUD, jobs, products, stock adjust, invoices CRUD, invoice email, payments list) as needed from previous versions

# Example invoice email (optional)
...
def send_invoice_email(id):
    ...
    msg = Message("Your AZEX PestGuard Invoice", recipients=[to_email])
    ...
    try:
        ...
        invoice.status = 'Sent'
        db.session.commit()
        return jsonify({'message': 'Invoice emailed'})
